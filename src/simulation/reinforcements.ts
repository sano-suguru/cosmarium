...
export function reinforce(dt: number) {
  ...
  setRT(rT + dt); if (rT < 2.5) return; setRT(0);
  for (var team = 0; team < 2; team++) {
    ...
    if (cnt < lim) {
      ...
      if (r < 0.50) s(2, 80);                  // Bomber — 50%
      if (r < 0.40) s(3, 80);                  // Cruiser — 40%
      if (cnt < 50 && r < 0.10) s(4, 80);      // Flagship — 10% (only when losing)
      if (r > 0.20 && r < 0.35) s(5, 60);      // Healer — 15% [0.20–0.35)
      if (r > 0.35 && r < 0.50) s(6, 60);      // Reflector — 15% [0.35–0.50)
      if (cnt < 40 && r < 0.18) s(7, 80);      // Carrier — 18% (only when losing)
      if (r > 0.50 && r < 0.65) s(8, 80);      // Sniper — 15% [0.50–0.65)
      if (r > 0.65 && r < 0.77) s(9, 50);      // Ram — 12% [0.65–0.77)
      if (r > 0.30 && r < 0.45) s(10, 60);     // Missile — 15% [0.30–0.45)
