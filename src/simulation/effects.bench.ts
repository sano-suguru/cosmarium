...
describe('explosion', () => {
  bench('小型 (Fighter)', () => {
    rng.reset();
    explosion(2000, 2000, 0, FIGHTER, rng);
    resetPools();
  });
  ...
  bench('大型 (連続10回)', () => {
    rng.reset();
    for (let i = 0; i < 10; i++) {
      explosion(2000 + i * 100, 2000, 0, FIGHTER, rng);
    }
    resetPools();
  });
});
...
describe('chainLightning', () => {
  bench('3ホップ', () => {
    ...
    if (killer) {
      chainLightning(2000, 2000, 0, 10, 3, [0.5, 0.8, 1], killer, rng);
    }
    resetPools();
  });
  ...
  bench('6ホップ', () => {
    ...
    const killer = captureKiller(src);
    if (killer) {
      chainLightning(2000, 2000, 0, 10, 6, [0.5, 0.8, 1], killer, rng);
    }
    resetPools();
  });
});
