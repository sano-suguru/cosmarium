...
import type { SquadronIndex, Team } from '../types.ts';
...
describe('assignToSquadron', () => {
  it('ユニットを新規分隊に配属し、リーダーに昇格する', () => {
    ...
    assignToSquadron(idx, 0 as Team);
  ...
  it('2番目のメンバーは均等配分で別分隊に配属される', () => {
    ...
    assignToSquadron(a, 0 as Team);
    assignToSquadron(b, 0 as Team);
...
describe('formSquadrons', () => {
  it('チーム内の全ユニットが分隊に配属される', () => {
    ...
    formSquadrons(0 as Team, getUnitHWM());
...
describe('squadron-overflow', () => {
  it('全分隊満員時に新ユニットは配属されない', () => {
    ...
    assignToSquadron(extra, 0 as Team);
...
describe('computeSquadronCohesion', () => {
  ...
  it('SQUADRON_COHESION_DIST 以内では追従力はゼロ', () => {
    const leader = spawnAt(0 as Team, 0, 0, 0);
    const member = spawnAt(0 as Team, 0, 50, 0); // 50 < SQUADRON_COHESION_DIST(80)
    setupColocatedSquadron(leader, member);
