...
describe('dt clamping', () => {
  it('rawDt > 0.033 はクランプされる', () => {
    ...
    expect(pP[0]!.life).toBeCloseTo(1.0 - 0.033);
  });

  it('rawDt <= 0.033 はそのまま使われる', () => {
    spP(0, 0, 0, 0, 1.0, 1, 1, 1, 1, 0);
    update(0.02, 0); // dt = min(0.02, 0.033) = 0.02
    expect(pP[0]!.life).toBeCloseTo(1.0 - 0.02);
  });
});
...
describe('パーティクル pass', () => {
  it('移動 + drag 0.97', () => {
    ...
    update(0.016, 0);
    expect(pP[0]!.x).toBeCloseTo(100 * 0.016, 1);
    // vx は drag 後: 100 * 0.97 = 97
    expect(pP[0]!.vx).toBeCloseTo(97);
    expect(pP[0]!.vy).toBeCloseTo(200 * 0.97);
  });
...
describe('projectile pass', () => {
  it('移動: x += vx*dt', () => {
    ...
    update(0.016, 0);
    // x = 0 + 300 * 0.016 = 4.8
    expect(prP[0]!.x).toBeCloseTo(4.8);
  });
  ...
  it('AOE 爆発: 範囲内の敵にダメージ + addShake(3)', () => {
    ...
    expect(uP[enemy]!.hp).toBeLessThan(10);
  ...
  it('homing: ターゲット生存時に追尾で曲がる', () => {
    ...
    update(0.016, 0);
    // vy が正方向に増加（上に曲がる）
    expect(prP[0]!.vy).toBeGreaterThan(0);
  });
