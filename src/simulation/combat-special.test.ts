...
describe('combat — LANCER', () => {
  it('衝突時に敵にダメージ (mass×3×vd) + 自傷 (敵mass×2)', () => {
    const lancerType = unitType(asType(9));
  ...
  it('敵HP<=0 → killUnit + explosion', () => {
    const lancer = spawnAt(0, asType(9), 0, 0);
    const enemy = spawnAt(1, asType(0), 5, 0); // Drone (hp=3, size=4)
    buildHash();
    combat(unit(lancer), lancer, 0.016, 0, rng);
  ...
  it('自身HP<=0 → 自身も死亡', () => {
    ...
    spawnAt(1, asType(4), 5, 0); // Flagship (mass=30)
    buildHash();
    combat(unit(lancer), lancer, 0.016, 0, rng);
  ...
  it('フィールド持ち相手に ram ダメージ半減', () => {
    const lancer = spawnAt(0, asType(9), 0, 0); // Lancer (mass=12)
    const enemy = spawnAt(1, asType(6), 5, 0); // Reflector (mass=3)
  ...
  it('reflectFieldHp が ramDmg 分減少', () => {
    const lancer = spawnAt(0, asType(9), 0, 0);
    const enemy = spawnAt(1, asType(6), 5, 0);
    unit(enemy).reflectFieldHp = REFLECT_FIELD_MAX_HP;
    buildHash();
    combat(unit(lancer), lancer, 0.016, 0, rng);
...
describe('combat — CARRIER', () => {
  it('spawnCooldown<=0 で Drone×4 スポーン', () => {
    const carrier = spawnAt(0, asType(7), 0, 0); // Carrier
  ...
  it('spawnCooldown > 0 → スポーンなし', () => {
    const carrier = spawnAt(0, asType(7), 0, 0);
...
describe('combat — DISRUPTOR', () => {
  it('範囲内の敵にstun=1.5 + ダメージ', () => {
    const disruptorType = unitType(asType(11));
    const disruptor = spawnAt(0, asType(11), 0, 0);
...
describe('combat — TELEPORTER (3連ブリンク)', () => {
  ...
  it('着地衝撃: 近接敵にノックバック + ミニスタン', () => {
    const tp = spawnAt(0, asType(13), 0, 0);
    const enemy = spawnAt(1, asType(1), 200, 0);
    const nearby = spawnAt(1, asType(0), 0, 0);
    unit(tp).teleportTimer = 0;
    unit(tp).target = enemy;
    buildHash();
    // 出発
  ...
  it('着地衝撃: 味方にはノックバック/スタンがかからない', () => {
    const tp = spawnAt(0, asType(13), 0, 0);
    const enemy = spawnAt(1, asType(1), 200, 0);
    const ally = spawnAt(0, asType(0), 0, 0);
    unit(tp).teleportTimer = 0;
    unit(tp).target = enemy;
    buildHash();
    // 出発
  ...
  it('着地衝撃: 効果範囲外の敵は影響なし', () => {
    const tp = spawnAt(0, asType(13), 0, 0);
    const enemy = spawnAt(1, asType(1), 200, 0);
    const farEnemy = spawnAt(1, asType(0), 0, 0);
    unit(tp).teleportTimer = 0;
    unit(tp).target = enemy;
    buildHash();
    // 出発
...
describe('combat — CHAIN LIGHTNING', () => {
  it('chainLightning() 呼出 + cooldownリセット', () => {
    const arcer = spawnAt(0, asType(14), 0, 0); // Arcer (rng=250, fireRate=2)
    const enemy = spawnAt(1, asType(1), 100, 0);
    ...
    // cooldown = fireRate = 2
    expect(unit(arcer).cooldown).toBeCloseTo(unitType(asType(14)).fireRate);
    // ビーム + ダメージ
    expect(beams.length).toBeGreaterThan(0);
  });

  it('tgt<0 → 即return', () => {
    const arcer = spawnAt(0, asType(14), 0, 0);
    unit(arcer).cooldown = 0;
