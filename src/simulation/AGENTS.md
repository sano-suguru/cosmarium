...
### combat分岐フロー
排他(`return`あり): rams → reflects → emp → chain → sweep → beam。非排他: heals, spawns, teleports。最後: NORMAL FIRE(homing/aoe/5-burst/railgun/default)。`tgtDistOrClear(u)`でターゲット検証+距離取得を一括処理。
...
| ファイル | 行数 | 役割 |
|---------|------|------|
| update.ts | 290 | tick本体。サブステップ分割、per-unit/projectile/particle/beam更新 |
| combat.ts | 870 | 排他/非排他の攻撃分岐。最大ファイル |
| steering.ts | 250 | Boids + ターゲット追跡 + 境界力 |
| effects.ts | 280 | explosion/trail/chain/boost エフェクト生成 |
| spawn.ts | 140 | spawn/kill集約。プール操作の唯一のエントリポイント |
| spatial-hash.ts | 70 | buildHash + getNeighbors（共有バッファ） |
| reinforcements.ts | 110 | 確率テーブルによる増援spawn |
| init.ts | 60 | INIT_SPAWNS。ゲーム開始時のユニット配置 |
...
- `combat()` の排他分岐に `sweep` を追加: rams → reflects → emp → chain → sweep → beam
- `dt`サブステップ分割: `dt > 1/REF_FPS` なら最大 `MAX_STEPS_PER_FRAME=8` 回の `stepOnce` に分割（クランプではなく分割）
