...
| effects.ts | explosion/trail/boost エフェクト生成、destroyUnit/destroyMutualKill |
...
- `stepOnce(SIM_DT)` が唯一のpublic API。main.ts の `drainAccumulator` が accumulator パターンで固定 dt 刻みの呼び出しを管理（最大8ステップ/フレーム）
...
- `getNeighbors()` は `NeighborSlice` を返す。`buildHash()` 後のみ有効。途中のユニット追加は反映されない。`nb.count` / `nb.at(i)` でアクセス
- `killUnit()` + `explosion()` のペアは必ず `destroyUnit()` でまとめること — 内部でスナップショットを取るため手動退避不要。`killUnit()`/`killParticle()`/`killProjectile()` はすべてスナップショットを返す
