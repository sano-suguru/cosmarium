...
最後: NORMAL FIRE — `homing` / `aoe` / `shape===3`(5-burst) / `shape===8`(railgun) / default

### ヘルパー関数

- `tgtDistOrClear(u)`: ターゲット距離を返す。`u.target<0` or `!alive` なら `u.target=-1` でリセットし-1を返す。combat冒頭でターゲット検証と距離取得を一括処理
...
### steering変更
- `steer()` の力の合成: `fx/fy` に各力を加算 → 最後に角速度`u.angle`に反映
...
- ターゲット選択: 近傍 → 失敗時1.2%で全域スキャン → なければ`wanderAngle`ワンダリング
- 距離による行動: 遠い(`>0.7×rng`)=接近、近い(`<0.3×rng`)=離脱、中間=側面旋回
- **Healer**: 近傍で最も`mass`が大きい味方に向かう力を追加
- **境界力**: `WORLD_SIZE*0.8`を超えるとfx/fy += 120で押し戻し
