...
import { CRUISER_TYPE, FIGHTER_TYPE, REFLECTOR_TYPE, SCORCHER_TYPE, unitType } from '../unit-types.ts';
...
describe('combat — BEAM REFLECT (リトロリフレクション)', () => {
  it('攻撃元に直接ダメージが返る', () => {
    const scorcher = spawnAt(0, SCORCHER_TYPE, -200, 0);
    const reflector = spawnAt(1, REFLECTOR_TYPE, 0, 0);
  ...
  it('第三者にはダメージが及ばない', () => {
    const scorcher = spawnAt(0, SCORCHER_TYPE, -200, 0);
    const reflector = spawnAt(1, REFLECTOR_TYPE, 0, 0);
    const bystander = spawnAt(0, FIGHTER_TYPE, 0, 200);
  ...
  it('Sweep beam + Reflector でバッファ競合なく動作する', () => {
    ...
    const reflector = spawnAt(1, REFLECTOR_TYPE, 200, 0);
  ...
  it('Sweep中にReflector反射でattackerが死亡 → 例外なく中断', () => {
    const cruiser = spawnAt(0, CRUISER_TYPE, 0, 0);
    const reflector = spawnAt(1, REFLECTOR_TYPE, 80, 0);
...
describe('combat — FIELD BEAM REFLECT (reflectFieldHp によるビーム反射)', () => {
  it('フィールド持ち味方がビームを反射しダメージを返却する', () => {
    const scorcher = spawnAt(0, SCORCHER_TYPE, -200, 0);
    const ally = spawnAt(1, FIGHTER_TYPE, 0, 0); // Drone (non-reflector)
  ...
  it('フィールドHPがビームダメージ分減少する', () => {
    const scorcher = spawnAt(0, SCORCHER_TYPE, -200, 0);
    const ally = spawnAt(1, FIGHTER_TYPE, 0, 0);
    unit(ally).reflectFieldHp = REFLECT_FIELD_MAX_HP;
  ...
  it('Reflector本体のenergy反射がフィールド反射より優先される', () => {
    const scorcher = spawnAt(0, SCORCHER_TYPE, -200, 0);
    const reflector = spawnAt(1, REFLECTOR_TYPE, 0, 0); // Reflector本体
  ...
  it('Sweep Beamがフィールドで反射される', () => {
    const cruiser = spawnAt(0, CRUISER_TYPE, 0, 0);
    const ally = spawnAt(1, FIGHTER_TYPE, 80, 0);
    unit(ally).reflectFieldHp = REFLECT_FIELD_MAX_HP;
    unit(ally).hp = 100;
