...
describe('production', () => {
  ...
  it('tickProduction は独立タイマーで各スロットを並行処理する', () => {
    setupMothership(rng);
    setMothershipVariant(TEAM0, 0 as MothershipVariant); // Hive: productionRateMul=0.7
  ...
  it('母艦が撃破されると全スロット生産停止', () => {
    setupMothership(rng);
    setMothershipVariant(TEAM0, 0 as MothershipVariant);
    const slots: (ProductionSlot | null)[] = [
  ...
  it('MAX_CLUSTERS_PER_TICK でスポーン回数がキャップされる', () => {
    setupMothership(rng);
    setMothershipVariant(TEAM0, 2 as MothershipVariant); // Reactor: productionRateMul=1.0
  ...
  it('computeProductionCap 未到達では通常生産', () => {
    setupMothership(rng);
    setMothershipVariant(TEAM0, 2 as MothershipVariant);
    const slots: (ProductionSlot | null)[] = [{ type: DRONE_TYPE, count: 1 }];
    const ps = initProductionState(slots);
...
describe('computeProductionCap', () => {
  ...
  it('母艦1体撃沈後にキャップが増加する（melee 動的再計算）', () => {
    ...
    for (let t = 0; t < 3; t++) {
      ...
      setMothershipVariant(t as Team, 0 as MothershipVariant);
    }
