...
export function killU(i: number) {
  if (uP[i].alive) { uP[i].alive = false; poolCounts.uC--; }
}

export function spP(x: number, y: number, vx: number, vy: number, life: number, sz: number, r: number, g: number, b: number, sh: number): number {
  for (var i = 0; i < PP; i++) {
    if (!pP[i].alive) {
      var p = pP[i];
      ...
      poolCounts.pC++; return i;
    }
  }
  return -1;
}

export function spPr(x: number, y: number, vx: number, vy: number, life: number, dmg: number, team: number, sz: number, r: number, g: number, b: number, hom?: boolean, aoe?: number, tx?: number): number {
  for (var i = 0; i < PPR; i++) {
    if (!prP[i].alive) {
      var p = prP[i];
      p.alive = true; p.x = x; p.y = y; p.vx = vx; p.vy = vy;
      ...
      poolCounts.prC++; return i;
    }
  }
  return -1;
}

export function addBeam(x1: number, y1: number, x2: number, y2: number, r: number, g: number, b: number, life: number, w: number) {
