...
export function Menu({ onStart, onSpectate, onMelee, onCodex }: MenuProps) {
  return (
    <div class={styles.menu}>
      ...
      <button type="button" class={btnStyles.btn} onClick={onStart}>
        START<span class={styles.desc}>艦隊を編成して宇宙戦を戦い抜け</span>
      </button>
      <button type="button" class={btnStyles.btn} onClick={onSpectate}>
        SPECTATE<span class={styles.desc}>終わりなき宇宙艦隊戦を観測せよ</span>
      </button>
      <button type="button" class={btnStyles.btn} onClick={onMelee}>
        MELEE<span class={styles.desc}>2〜5勢力のランダム乱戦を観戦せよ</span>
      </button>
      <button type="button" class={btnStyles.btn} onClick={onCodex}>
