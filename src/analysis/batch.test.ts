...
describe('runBatch', () => {
  ...
  describe('fleet count validation', () => {
    it('battle モードで fleets.length !== 2 のときエラー', () => {
      ...
      expect(() => runBatch(makeTestConfig({ mode: 'battle', fleets: [fleet] }))).toThrow(
        'fleets.length (1) must equal the number of active teams (2)',
      );
    });

    it('melee モードで fleets.length !== teams のときエラー', () => {
      const fleet: FleetComposition = [{ type: asType(0), count: 3 }];
      expect(() => runBatch(makeTestConfig({ mode: 'melee', teams: 3, fleets: [fleet, fleet] }))).toThrow(
        'fleets.length (2) must equal the number of active teams (3)',
      );
    });
