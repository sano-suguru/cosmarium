...
function mmW(x: number, y: number, sx: number, sy: number, r: number, g: number, b: number, a: number, sh: number) {
  ...
  var B = mmIc * 9;
  mmD[B]=x; mmD[B+1]=y; mmD[B+2]=sx;
  mmD[B+3]=r; mmD[B+4]=g; mmD[B+5]=b; mmD[B+6]=a;
  mmD[B+7]=sy; mmD[B+8]=sh;
  mmIc++;
}
...
export function initMinimap() {
  ...
  mmDiv.addEventListener('wheel', function(e) {
    e.preventDefault();
  }, { passive: false });
}
