...
describe('getColor', () => {
  it('index 0, team 0 を返す', () => {
    expect(color(0, 0)).toEqual([0.2, 1, 0.55]);
  });

  it('index 0, team 1 を返す', () => {
    expect(color(0, 1)).toEqual([1, 0.45, 0.25]);
  });

  it('index 14 (最後), team 0 を返す', () => {
    expect(color(14, 0)).toEqual([1, 1, 0.3]);
  });

  it('index 14, team 1 を返す', () => {
    expect(color(14, 1)).toEqual([0.3, 1, 1]);
  });
...
describe('getTrailColor', () => {
  it('index 0, team 0 のトレイルカラーを返す', () => {
    expect(trailColor(0, 0)).toEqual([0.1, 0.6, 0.35]);
  });

  it('index 0, team 1 のトレイルカラーを返す', () => {
    expect(trailColor(0, 1)).toEqual([0.6, 0.25, 0.12]);
  });
