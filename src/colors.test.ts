...
describe('gC', () => {
  it('index 0, team 0 を返す', () => {
    expect(gC(0, 0)).toEqual([0.2, 1, 0.55]);
  });

  it('index 0, team 1 を返す', () => {
    expect(gC(0, 1)).toEqual([1, 0.45, 0.25]);
  });

  it('index 14 (最後), team 0 を返す', () => {
    expect(gC(14, 0)).toEqual([1, 1, 0.3]);
  });

  it('index 14, team 1 を返す', () => {
    expect(gC(14, 1)).toEqual([0.3, 1, 1]);
  });
...
describe('gTr', () => {
  it('index 0, team 0 のトレイルカラーを返す', () => {
    expect(gTr(0, 0)).toEqual([0.1, 0.6, 0.35]);
  });

  it('index 0, team 1 のトレイルカラーを返す', () => {
    expect(gTr(0, 1)).toEqual([0.6, 0.25, 0.12]);
  });
